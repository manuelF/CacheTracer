...
KNOB<string> KnobsizeL1(KNOB_MODE_WRITEONCE, "pintool",
    "L1", "16", "Especifica la cantidad de bits que tiene una direccion en la L1 (tamaño)");

KNOB<string> KnobsizeL2(KNOB_MODE_WRITEONCE, "pintool",
    "L2", "20", "Especifica la cantidad de bits que tiene una direccion en la L2 (tamaño)");

KNOB<string> KnobVias(KNOB_MODE_WRITEONCE, "pintool",
    "V", "4", "Especifica la cantidad de vias que hay en la cache");

KNOB<string> KnobLine(KNOB_MODE_WRITEONCE, "pintool",
    "L", "4", "Especifica la cantidad de bits que tiene una direccion de memoria que se usan para indexar dentro de la linea (tamaño)");

KNOB<string> KnobOutputFile(KNOB_MODE_WRITEONCE, "pintool",
    "o", "cache.out", "Especifica el nombre del archivo de salida del informe");

KNOB<string> KnobLRU(KNOB_MODE_WRITEONCE, "pintool",
    "lru", "0", "Utiliza desalojo LRU de la linea en vez de FIFO");
...
void accessCache(UINT32 writeAccess, UINT64 address, UINT32 cacheLevel)
{
    ...
    if(writeAccess) cacheRead[cacheLevel]++;
    else            cacheWrite[cacheLevel]++;
    ...
    //Encontramos ese tag, vamos a marcar el hit y anotamos el dirty si es necesario
    if(tagEncontrado)
    {
        cacheHit[cacheLevel]++;
        if(writeAccess)
                L[indice_encontrado].dirty=true;

    }
    //hubo miss en este nivel, ahora vemos si podemos traer de un nivel superior o de mas arriba
    else
    {
        //Es miss en este nivel
        cacheMiss[cacheLevel]++;

        //Vamos a hacer una lectura en los caches superiores a ver si lo encontramos
        ...
        if(i!=sizeVia) //encontramos un lugar vacio; contamos que vamos a guardar el dato ahi
        {
        ...
        else //todo lleno, vamos a desalojar
        {
            ...
            if(L[nuevo_indice].dirty) //si el desalojamos, habia sido escrito, hay que propagarlo
            {
...
VOID Instruction(INS ins, VOID *v)
{
    ...
    // Por cada operando de memoria, vemos si es lectura o escritura y llamamos a la
    // funcion correspondiente para contabilizar
    for (UINT32 memOp = 0; memOp < memOperands; memOp++)
    {
        if (INS_MemoryOperandIsRead(ins, memOp))
        {
            INS_InsertPredicatedCall(
                ins, IPOINT_BEFORE, (AFUNPTR)memAccessR,
        ...
        {
			INS_InsertPredicatedCall(
                ins, IPOINT_BEFORE, (AFUNPTR)memAccessW,
                IARG_MEMORYOP_EA, memOp,
                IARG_END);
		}
...
// Fini se llama cuando se sale del programa y termina el sim, e imprime el informe
VOID Fini(INT32 code, VOID *v)
{
        UINT32 i; //iterador de lineas
        UINT32 level; //iterador de niveles de cache
        UINT32 lineasUsadas[CACHE_LEVELS]; // contador de lineas usadas en cada nivel de cache
        UINT32 celdasUsadas[CACHE_LEVELS]; // contador de vias usadas en cada nivel de cache

        // Escribimos a un archivo porque el programa puede modificar cerr y cout normalmente
        ofstream OutFile;
        OutFile.open(KnobOutputFile.Value().c_str());
        OutFile.setf(ios::showbase);
        char niveles[10];
        sprintf(niveles, "%d",CACHE_LEVELS);
        string title = "Analisis de cache writeback para  \"" + executableParameter +"\" usando " + string(niveles) + " niveles de cache usando reemplazo " ;
        if(lru==1)
            title += "LRU";
        else
            title += "FIFO";
        ...
        {
            OutFile << "Tamaño de L" <<level+1 << ": "  << size[level]*sizeVia/1024 << "kb ";
            OutFile << "Cantidad de lineas de L" <<level+1 << ": " << size[level] << endl;
        }
        OutFile << endl;
        ...
        for(level = 0 ; level<CACHE_LEVELS; level++)
        {
                ...
                OutFile << "Hitrate de L" << level+1  <<": " << ((double)cacheHit[level] / (double)(cacheHit[level]+cacheMiss[level]))*100 << "%" << endl;
                OutFile << "Cantidad de Hits en L" << level+1  <<": " << cacheHit[level] << endl;
                OutFile << "Cantidad de Miss L" << level+1  <<": "  << cacheMiss[level] << endl;
                OutFile << "Cantidad de Read L" << level+1  <<": "  << cacheRead[level] << endl;
                OutFile << "Cantidad de Write L" << level+1  <<": "  << cacheWrite[level] << endl;
                OutFile << "Cantidad de WriteBacks al siguiente nivel de L" << level+1  <<": " << cacheWriteBacks[level] << endl;
                OutFile << endl;
        }
        OutFile << endl;
        for(level = 0 ; level<CACHE_LEVELS; level++)
        {
                ...
                OutFile << "Lineas de L" <<level+1<<" usadas: " << lineasUsadas[level] << " de " <<  size[level]<< endl;
        }
        OutFile << endl;
        ...
        for(level = 0 ; level<CACHE_LEVELS; level++)
        {
                ...
                OutFile << "Lineas Internas (Lineas * Vias) de L"<< level+1<<" usadas: " << celdasUsadas[level] << " de " << size[level]*sizeVia << endl;
        }
        OutFile.close();
        freeBuffers();
}
...
INT32 Usage()
{
    cerr << endl << "Esta herramienta simula una cache de CPU parametrizable al correr un programa" << endl;
    cerr << endl << KNOB_BASE::StringKnobSummary() << endl;
    return -1;
}

//Buscamos el delimitador del parametro nombre de archivo, para poner en el informe
...
int main(int argc, char *argv[])
{
    ...
    //cargamos de los parametros el path del ejecutable para ponerlo en el informe
